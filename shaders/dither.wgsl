...
fn sobel(pixel_position: vec2<f32>) -> f32 {
    ...
    for (var x: i32 = -1; x <= 1; x++) {
        for (var y: i32 = -1; y <= 1; y ++) {
            let uv = (pixel_position + vec2<f32>(f32(x), f32(y))) / vec2<f32>(rect.zw);
            let sample = simplex_3(vec3<f32>(uv.x * 2.0, uv.y * 2.0, time * 0.35));
...
fn main(@location(0) uv: vec2<f32>, @builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    let noise = simplex_3(vec3<f32>(uv.x * 2.0, uv.y * 2.0, time * 0.35));
